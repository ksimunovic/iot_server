...
import org.foi.nwtis.karsimuno.podaci.Zahtjev;
...
@RequestScoped
@ManagedBean(name = "pregledZahtjeva")
public class PregledZahtjeva {
    ...
    public List<Zahtjev> getZahtjevi() {
        ...
        return dohvatiZahtjeve();
    }

    private List<Zahtjev> dohvatiZahtjeve() {
        List<Zahtjev> temp = new ArrayList();
        baza = new BazaHelper();
        try {
            conn = baza.spojiBazu();
            ...
            while (rs.next()) {
                temp.add(new Zahtjev(rs.getInt("id"), rs.getString("korisnik"), rs.getString("naredba"), rs.getString("odgovor"), rs.getTimestamp("vrijeme")));
            }
    ...
    private void prebrojiZapise() {
        ...
        try {
            ...
            String sql = "SELECT count(*) FROM zahtjevi";
            stmt = conn.prepareStatement(sql);
            rs = stmt.executeQuery();
